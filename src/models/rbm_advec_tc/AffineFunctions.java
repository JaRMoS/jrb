...
public class AffineFunctions implements IAffineFunctions, ITransient, IAffineInitials {
	...
	public double thetaQl(int k, int i, double[] p, double t) {
		if ((k < 0) || (k > getQl().length - 1)) {
			throw new RuntimeException("Input parameter is invalid in evaluateL(),"
					+ " k = " + k + " but get_n_outputs() = " + getQl());
		}

		if ((i < 0) || (i > getQl()[i] - 1)) {
			throw new RuntimeException("Input parameter is invalid in evaluateL(),"
					+ " q_l = " + i + " but get_Q_l(i) = " + getQl()[i]);
		}
	...
	public double thetaQf(int i, double[] p, double t) {
		if ((i < 0) || (i > getQf() - 1)) {
			throw new RuntimeException("Input parameter is invalid in evaluateF(),"
					+ " i = "
					+ i
	...
	public double thetaQa(int i, double[] p, double t) {
		if ((i < 0) || (i > getQa() - 1)) {
			throw new RuntimeException("Input parameter is invalid in evaluateA(),"
					+ " i = "
					+ i
